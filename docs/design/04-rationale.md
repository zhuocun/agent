...
## Decision 16: Welcome-hero atmosphere — gradient, display serif, composer glow (welcome-only)

**Choice:** The first-run welcome surface gains a token-level atmosphere kit: `--hero-gradient` (a static brand-hue wash layered beneath the existing `--welcome-ambient` bloom), `--hero-glow-edge` / `--hero-glow-halo` (a resting composer halo composed from the `--focus-glow-*` recipe at a fraction of its activated energy), a display serif (Instrument Serif via `next/font`, exposed as `--font-heading`) for the hero greeting at display sizes only, and `--radius-3xl` so the welcome surface's generous rounding tracks the system radius knob. Every piece is welcome-only; the working thread is untouched.

**Anti-pattern exception, named:** atmospheric gradients are a catalogued violation on the working surface — `03-anti-patterns.md` §G ("Personality bleeding into the working surface") names "atmospheric gradients or hero typography that the welcome state uses correctly, repeated on the working thread" as the failure, and §C forbids gradients that *drift*. This decision exercises the carve-out those entries already anticipate: the gradient and hero typography live on the welcome state exclusively, and the gradient is static (no ambient motion). Any component that reaches for `--hero-gradient`, `--hero-glow-*`, or `--font-heading` outside the welcome surface should fail review under §G.

**Alternatives considered:**

- *No atmosphere beyond the existing `--welcome-ambient` bloom* — the status-quo position. Rejected because Tension 5 and Decision 11 allocate the product's entire distinctiveness budget to the welcome seam; a single radial bloom under-spends it, and the budget cannot be spent anywhere else.
- *Gradient on the thread header or composer as well* — "ambient brand" in low doses. Rejected outright; this is the exact §G violation the carve-out exists to fence off.
- *A second hue in the hero gradient* — a warm or neighboring cool tint for atmospheric range. Rejected because the single-accent doctrine (Decision 02) has no welcome-state exemption: the gradient stays on brand hue 247 over the neutral 250 base, only alpha varies.
- *Serif as the welcome body face too* — extend the serif to the prompt-card labels. Rejected; the prompt cards are working affordances, and Decision 04's hierarchy rule (weight, size, spacing — not family swaps) governs everything below display size.
- *`display: "swap"` for the serif* — guarantees the serif always lands. Rejected because a late swap is a layout shift at first paint, which the renderer contract (PRD 01 §5.4) prohibits and Decision 04 names as the FOUT failure. `optional` means some cold-cache first visits render the greeting in the metric-adjusted serif fallback — accepted.
- *Reusing `--focus-glow-*` directly behind the resting composer* — literal token reuse. Rejected because Decision 07 makes the activated focus glow the *single* moment of full accent illumination; a full-energy glow at rest would dilute the engagement signal. The hero glow is the same hue and geometry at reduced alpha, so rest→focus reads as the same light turning up.

**Why this choice:** Decision 11 gives the welcome state one job — introduce the product — and Tension 5 rules that distinctiveness belongs to exactly this seam. The existing bloom carries light; the gradient adds atmosphere; the serif gives the greeting a voice the system stack cannot; the composer glow points the user at the one action the surface wants. All four are tokens, not component styling: both themes define every token (dark is designed, not flipped — High Aesthetics' parity rule), `prefers-contrast: more` zeroes the gradient and glow exactly as it zeroes `--welcome-ambient`, and the glow tokens zero to a no-op shadow rather than `none` so composed box-shadow lists stay valid. The serif exercises Decision 04's own revisit clause ("a first-run surface needs a display face for a single moment... the budget can be confined to that surface"): `next/font` self-hosts the file, `display: optional` plus the metric-adjusted fallback keep it off the critical path with zero CLS, and the working surface never sees it.

**Trade-off accepted:** A small font payload (~one woff2, single 400 weight) ships for a surface the user may see only once per session — and on cold-cache first visits `display: optional` may render the greeting in the fallback serif instead. The welcome surface is also now visually farther from the thread, which raises the bar on the welcome→thread exit choreography to keep the seam feeling like one product.

**Revisit when:** A second seam (marketing entry, share-page header) earns a hero treatment and needs these tokens to generalize beyond the welcome surface — generalize the *tokens*, not the carve-out — or LCP/bandwidth telemetry on mobile-web shows even the non-blocking font payload measurably hurting first paint.
