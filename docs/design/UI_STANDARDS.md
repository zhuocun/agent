...
**C38 — The spacing grid admits a 2 px sub-step inside controls. RESOLVED.** `docs/prd/06-design-system-visual-spec.md` §3.3 said only "4px base grid", and a first reading of UI-CRAFT-1 applied that to every box. The build carries 256 half-step spacing utilities across 49 files under `web/src/components`, resolving to 2, 6 or 14 px, and most sit inside a control: `buttonVariants` sets `gap-1.5 px-2.5`, and the `.chat-md` inline-code rule sets `px-1.5 py-0.5`. A base grid sets the rhythm between components, not a ban on finer steps inside one; Tailwind's own spacing scale ships the half-steps for that purpose. **Resolution:** PRD 06 §3.3 now carries a bullet admitting 2 px sub-steps up to 14 px inside a single control or inline chip, and UI-CRAFT-1 asserts exactly that, branching on the control's selector. Half-steps between components or in a container gutter remain findings against UI-CRAFT-1; they have not been counted here, so each one a reviewer finds is a fresh defect, not a re-filing. The audit harness in `web/tests/audit/probes.ts` admits 2 px steps up to 16 px everywhere, which is looser than the clause; its report is triage, not the gate.
...
1. Read §15 first. Several entries there name a `[must]` clause that the current build fails for a reason already diagnosed — C13 through C25, C29 through C37, C39 through C45, plus C48 — and re-filing one of those as a fresh defect wastes the review. An entry marked RESOLVED is closed and needs no attention.
