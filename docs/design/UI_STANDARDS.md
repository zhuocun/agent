...
**C50 — An inline citation cannot be 44 px tall inside 28 px leading.** UI-TOUCH-1 asks every control on a `hover: none` device for a 44×44 px hit region. A citation marker (`[1]`) sits inside running prose whose line box is 28 px (`.chat-md` `leading-7` in `web/src/app/globals.css`). A 44 px-tall hit region reaches about 9 px into the lines above and below, where it takes taps from links and from a citation on the neighbouring line, which UI-TOUCH-3 forbids. WCAG 2.2 SC 2.5.8 exempts a target "in a sentence" from its size floor for the same reason. **Ruling:** an inline citation's touch hit region is at least 44 px wide and exactly its line box tall; `web/src/components/chat/markdown-renderer.tsx` implements it and `web/tests/e2e/touch-targets.spec.ts` asserts it. UI-TOUCH-3 wins over UI-TOUCH-1 on height for inline targets only; every other control keeps the 44×44 floor. A reviewer must not file the citation's height against UI-TOUCH-1.
