...
**Assertion.** A 300-character string with no break opportunity — a pasted URL, a hash, a base64 blob — wraps inside the box that holds it, in a user message and in assistant prose. A 200-character conversation title truncates inside its sidebar row. None of the three widens its container past the message column or the row.
...
**Assertion.** Every element that visibly truncates its text — `text-overflow: ellipsis`, or a line clamp that is actually clipping — exposes the full string through a `title` on itself or an ancestor, or through a tooltip bound to its nearest interactive ancestor. That ancestor's accessible name does not contradict the full string.
**Source.** WCAG 2.2 SC 1.4.10 Reflow (AA), whose "without loss of information" condition a truncation meets only when the full text stays available; shipped precedent in `web/src/components/chat/sidebar.tsx`, which puts `title={conversation.title}` on the truncated row title.
...
**Assertion.** Every tooltip opens on keyboard focus as well as on pointer hover. It closes on Escape without moving focus, and stays open while the pointer travels from its trigger onto it. Its text adds nothing that the trigger's accessible name, or a visible surface such as the shortcuts dialog, does not also carry.
...
**Assertion.** The document has exactly one `h1`. In document order, no heading is more than one level deeper than the heading before it. An open dialog restarts the count at its own title.
...
**Assertion.** Every margin, padding and gap in rendered app markup resolves, at the default root size, to 0, to a 1 px hairline offset, or to a multiple of 4 CSS px. No component writes an arbitrary spacing value other than a composition of `env(safe-area-inset-*)` or `--bottom-inset` with an on-grid floor.
**Source.** `docs/prd/06-design-system-visual-spec.md` §3.3 ("4px base grid"), which wins on the value under §0's precedence rule; `docs/design/01-foundations.md`, Spacing → "A strict base grid, generous gutters" ("every gutter is a deliberate multiple"); `docs/design/00-principles.md`, Tension 4 (the spacing scale is "defined once in PRD 06 and not negotiated per surface").
**Verify.** (1) `rg -nP '(?<![\w-])-?(p[xytrblse]?|m[xytrblse]?|gap(-[xy])?|space-[xy])-\[' web/src/components`. Every hit must contain `safe-area-inset` or `--bottom-inset`. (2) Playwright at 390 px and at 1280 px, on the welcome surface, a thread and each dialog: for every rendered element read `marginTop`/`Right`/`Bottom`/`Left`, `paddingTop`/`Right`/`Bottom`/`Left`, `rowGap` and `columnGap`, and assert each pixel value `v` satisfies `v === 0 || Math.abs(v) === 1 || Math.abs(Math.abs(v) / 4 - Math.round(Math.abs(v) / 4)) < 0.01`. Headless Chromium resolves every inset to 0, so a safe-area composition reduces to its floor and needs no exemption. The first half passes today. The second fails on Tailwind half-steps — see §15 C38 before filing.

### UI-CRAFT-2 [must] — Radius comes from the token ladder
**Assertion.** Every rounded corner resolves through `--radius`: `rounded-sm`, `-md`, `-lg`, `-xl`, `-2xl` or `-3xl` and their per-corner forms; `rounded-full` for capsules and circles; `rounded-none`; or `rounded-[inherit]`. No component uses bare `rounded` or a bare per-corner form such as `rounded-t`, uses `rounded-xs` or `rounded-4xl`, or writes an arbitrary radius. Each of those compiles to a fixed length that ignores the radius knob.
...
**Why bare `rounded` fails.** It reads like a token and is not one. `globals.css` overrides the named steps in `@theme inline` and never the unsuffixed utility, so Tailwind's default theme inlines a literal for it. The built stylesheet has `.rounded{border-radius:.25rem}` beside `.rounded-lg{border-radius:var(--radius)}`.
**Verify.** (1) `rg -nP '(?<![\w-])rounded(-(t|r|b|l|s|e|tl|tr|br|bl|ss|se|es|ee))?(?=["'"'"'\x60\s])' web/src/components`. A hit inside a comment is prose ("rounded top", "rounded corners") and is discarded by reading the line; any hit in a class string fails. (2) `rg -nP '(?<![\w-])rounded(-(t|r|b|l|s|e|tl|tr|br|bl|ss|se|es|ee))?-(xs|4xl|\[(?!inherit\]))' web/src/components` returns nothing. Both fail today — §15 C39.

### UI-CRAFT-3 [should] — Elevation is composed from shadow tokens
**Assertion.** Every `box-shadow` a component writes is a shadow token — `shadow-glass-*`, `shadow-float`, `shadow-pill`, `shadow-focus-*`, `shadow-hero-*`, `var(--focus-ring)` or a `glass-*` utility — or an arbitrary stack whose colors all come from a token `var()`. No stock Tailwind elevation utility (bare `shadow`, `shadow-xs` through `shadow-2xl`) appears.
...
**Assertion.** Every icon inside a control is a Lucide glyph (`svg.lucide`) whose stroke, or fill for a filled glyph, resolves to the control's text color. This includes vendored chrome that accepts an icon override. No component draws an inline `<svg>`, and no second icon package is a dependency.
...
**Scope.** Icons only. A rendered Mermaid diagram is message content, and the brand mark under `web/public/` is an asset Decision 05 places outside the Lucide grid. Vendored chrome is in scope when the vendor accepts an icon override, because a second family then ships by this repo's choice, not the vendor's.
**Verify.** (1) `rg -n '<svg' web/src/components` returns nothing, and `web/package.json` names no icon package besides `lucide-react`. (2) Playwright at 1280 px on a finished `RICH_MARKDOWN:` turn, which renders the code-block chrome: `page.$$eval('button svg, a svg, [role="button"] svg, [role="menuitem"] svg', els => els.filter(s => { const cs = getComputedStyle(s); return !s.classList.contains("lucide") || ![cs.stroke, cs.fill].includes(cs.color); }).length)` returns 0. Fails today on the code-block controls — §15 C40.
...
**Verify.** Playwright at 390 px with touch and at 1280 px. For each cluster root — `header`, `[role="toolbar"][aria-label="Message actions"]`, each open `[role="menu"]`, and the nearest common ancestor of `[data-testid="composer-more-actions"]` and `[data-testid="composer-send"]` — collect the visible `svg.lucide` descendants. Assert `new Set(svgs.map(s => s.getBoundingClientRect().width.toFixed(1))).size === 1`, and that the set of `getAttribute("stroke-width")` values has one member between 2 and 2.25.

### UI-CRAFT-6 [should] — Every control has distinct hover and disabled renderings
**Assertion.** On a pointer device, every enabled interactive control changes visibly on hover in at least one of background, text color, border color, shadow, opacity, transform or text decoration. Every disabled control is dimmed to an opacity of 0.5 or less and does not respond to the pointer. Focus-visible is UI-FOCUS-2's and pressed is UI-TOUCH-7's; this clause completes the set.
...
**Scope.** Links inside `.chat-md` are excluded. They are prose, they carry an underline at rest, and their styling is Streamdown's.
**Verify.** Playwright at 1280 px without touch. For every visible, enabled `button, a[href], [role="button"], [role="tab"], [role="menuitem"], [role="option"], [role="switch"], [role="checkbox"]` outside `.chat-md`, snapshot the computed `backgroundColor`, `color`, `borderColor`, `boxShadow`, `opacity`, `transform`, `textDecorationLine` and `filter`. Call `locator.hover()`, wait 350 ms (the longest shipped control transition is 280 ms), snapshot again, and assert at least one value differs. Then, for every `:disabled`, `[aria-disabled="true"]` or `[data-disabled]` control, assert that its own computed `opacity`, or that of the nearest ancestor carrying the disabled state, is ≤ 0.5, and that its `pointer-events` is `none` or its `cursor` is `not-allowed` or `default`.

### UI-CRAFT-7 [should] — An async action shows it is pending and cannot fire twice
**Assertion.** A control that starts a server round-trip other than a chat turn — branch, save, share, delete, key test — switches within 100 ms of activation to a pending rendering, disabled and showing a spinner or a pending label. It ignores further activation until the request settles.
**Source.** `docs/design/00-principles.md`, High Aesthetics (the four-designed-states bullet); `docs/prd/03-mobile-cross-platform.md` §4.4 ("sub-100ms visual feedback regardless"); the shipped idiom in `web/src/components/chat/message-actions.tsx`, where Branch swaps to `Loader2` and "Branching" and sets `disabled` while `isBranching`.
...
**Assertion.** Hovering or keyboard-focusing any control leaves the layout box — offset position and size — of that control, its parent and its siblings unchanged. Hover and focus feedback is paint or transform only.
...
**Assertion.** Every z-index in component code comes from the documented layer scale: base content 0–30, modals 50, contextual popovers 60, toasts 70, status strip 100. A menu, tooltip or toast opened while a modal is open paints above it. Every open overlay's box lies inside the viewport at 320 px wide.
...
**Assertion.** At most one persistent banner is visible in the thread chrome at a time. When the temporary-chat banner and the degraded-status banner are both active, one of them yields.
...
**C38 — The spacing grid has half-steps the PRD does not admit.** UI-CRAFT-1 `[must]` takes its 4 px grid from `docs/prd/06-design-system-visual-spec.md` §3.3 and `docs/design/01-foundations.md` "A strict base grid, generous gutters". The build carries 256 half-step spacing utilities across 49 files under `web/src/components` — `mt-0.5`, `gap-1.5`, `py-1.5` and `px-3.5` lead — and each resolves to an offset of 2, 6 or 14 px. Many come from the shadcn primitives rather than from product code: `buttonVariants` sets `gap-1.5 px-2.5`, and the `.chat-md` inline-code rule sets `px-1.5 py-0.5`. Every arbitrary spacing value in the same tree is a safe-area composition, and those pass. The audit harness in `web/tests/audit/probes.ts` admits 2 px steps up to 16 px, which is laxer than the clause. **Ruling:** the clause stands as written, because the PRD wins on values. The remedy is either to convert the sites or to amend PRD 06 §3.3 to admit a 2 px sub-step inside controls, and then relax UI-CRAFT-1 to match. That is a design call. Until it is made, a reviewer must not re-file the existing half-step sites one by one.

**C39 — Six radius sites sit off the token ladder.** UI-CRAFT-2 `[must]` requires every corner to come from the `--radius-*` ladder. Tailwind v4 compiles a bare `rounded` to a fixed `.25rem` rather than to a token. Four sites use it: `rounded-t` at `web/src/components/settings/spend-analytics-panel.tsx:230`, `rounded` at `web/src/components/chat/sources-panel.tsx:218` and `:232`, and `rounded` at `web/src/components/chat/markdown-renderer.tsx:124`. Two primitives use arbitrary radii: `rounded-[4px]` at `web/src/components/ui/checkbox.tsx:16`, and `rounded-[min(var(--radius-md),10px)]` / `rounded-[min(var(--radius-md),12px)]` in `web/src/components/ui/button.tsx` at lines 25, 26, 30 and 32. At the default radius knob the button caps equal `--radius-md`, so nothing looks wrong today; the cap only bites when the knob is raised. **Ruling:** the clause fails on these sites. Moving a bare `rounded` to `rounded-sm` changes a pixel value, so it is a visual change, and it is not made here.
...
1. Read §15 first. Several entries there name a `[must]` clause that the current build fails for a reason already diagnosed — C13 through C25, C29 through C37, plus C38 through C45 — and re-filing one of those as a fresh defect wastes the review. An entry marked RESOLVED is closed and needs no attention.
