...
- **Subagent-scoped** stream events + typed message parts + a **per-run cost meter** (orchestrator emits estimate @ plan pause, progress ticks as workers complete, and final; wire schema carries `confidence`/`phase`) + per-subagent model attribution persistence.
...
- Resumable replay of an interrupted agentic run beyond the shipped `RESUMABLE_STREAMS_ENABLED` path (an agentic run reuses the same `Stream` reconciliation; mid-fan-out *replay* — reattaching a reconnecting client to a fan-out already in flight — is not extended here, known hard gap, see plan 02. Worker tool-HITL **pause/resume** itself is shipped: BE-005 wait-siblings + continuation, see **Remaining gaps**). Replay **semantics** are reused as-is; agentic runs **multiply** event/byte buffer caps by `AGENTIC_RESUMABLE_BUFFER_MULTIPLIER` (default 4). Truncation/coalescing remains an operational tuning concern if fan-out still overflows the multiplied bound.
...
Remaining FE gap (see **Remaining gaps**): the high-cost composer hint (FR-26f).
...
- **Cost is a sum, not a single usage chunk.** The terminal cost is the **sum of planner + workers + aggregator (+ real verifier when metered)**. Each phase prices on the binding that served it, the mid-flight ledger is `planner_cost + sum(worker costs)` (`orchestrator.py:2942`, `:3090-3093`; the resumed roll-up carries the same sum at `:1806-1809`), and planner spend is seeded across a plan-approval pause as `plannerCostUsd` (`orchestrator.py:2390`, `:2521`) so a pause / decline / admit-reject exit still bills it (FL-21; `test_agentic_budget.py::test_deep_research_admit_reject_reports_budget_halted`). Per-subagent markers persist nested attribution; the top-level `cost_usd` is the roll-up the handler emits. For the roll-up's status, see **Remaining gaps**.
...
- `subagent_done` — **wire schema** (`stream_events.py`): `{ subagentId, label?, role?, costUsd?, outcome?, attribution?, substitution?, … }`.
- `run_cost` — `{ subtotalUsd, capUsd, confidence?, phase?, … }`. **Orchestrator:** estimate @ plan pause (`confidence: estimate`, `phase: plan`), progress ticks as workers complete (`phase: progress`), final at completion.
...
- `error` — unchanged envelope (PRD 08). A per-run budget halt produces a **partial synthesis** + `done`, not an `error` (graceful degrade, FR-26g). Partial labeling is inline prose in the answer **plus** the typed PRD 08 `severity: "warning"` chip (see **Remaining gaps**).
...
4. **Enforces bounds.** A recursion-depth bound (default 1 orchestrator→worker level; workers do **not** spawn unbounded sub-trees — depth 1 by construction; runtime assert is target) and the per-run USD cap, checked against the planner-inclusive actuals from `pricing.py` as each `SubagentDone` lands (`orchestrator.py:2942`, `:3090-3093`). On cap breach (**mid-flight kill**): stop spawning, cancel unfinished tasks, and proceed to aggregate the completed workers' outputs — a partial synthesis, labeled inline **plus** the PRD 08 warning chip.
...
7. **Finalizes.** The handler computes the run-total attribution, persists the subagent-grouped `parts`, and yields `terminal`.
...
- **Mid-flight kill.** Because reservation is only an estimate, actual cost is checked against the cap as each **worker** `SubagentDone` lands. On breach, the orchestrator cancels unfinished tasks and proceeds to aggregate whatever completed — the graceful-degrade path above. The orchestrator also yields a mid-run `run_cost` progress tick per worker done. When the verifier is on, per-sample judge costs fold into the meter.
...
| Gap | Status | Notes |
| --- | --- | --- |
| Per-subagent `ModelAttribution` + substitution | **SHIPPED** | **Persisted** on `SubagentPart` and rendered live + after reload, on every route that actually served — planner, superseded sibling and HITL-paused worker included. Fuller requested→served+reason callouts remain open (plan 02); the per-worker FE disclosure has no E2E spec yet (plan 02 deferrals) |
| `subagent_done` full attribution + outcome on wire | **SHIPPED** | Every started subagent reaches a terminal `subagent_done` — including superseded and budget-cancelled worker pauses — carrying `attribution` whenever that phase observed non-zero usage or cost (a zero-usage phase omits it by design). Stop/disconnect cannot stream one by design (the generator is `aclose`d), so the handler repairs the persisted row; the FE validates the outcome on both the live and reload path instead of defaulting to `succeeded` |
| `execute_tool` OTel spans in `agent_loop.py` | **SHIPPED** | `execute_tool_span` wrapped around tool execution in `agent_loop.py` |
| `invoke_workflow` run root + span conventions | **SHIPPED** | One `invoke_workflow` span per agentic run is the parent of every phase span (`observability/tracing.py`, opened in `run_orchestrator`). It pins the GenAI semconv revision it targets, and records fan-out width, observed depth, retries, the `stop_reason` / `counted_event` / `run_outcome` triple, reserved-vs-actual USD and token classes — never content. The root is parented by hand rather than made current, because an async generator's scope crosses contexts and `start_as_current_span` there logs `Failed to detach context` on every stopped turn |
| Multi-sample (`N>1`) verifier spans indistinguishable | **SHIPPED (closed)** | Each judge sample's `invoke_agent` span carries `agentic.sample_index` and a suffixed `agentic.sample_id`; `agentic.subagent_id` still holds the id the wire and ledger use. Was listed as a known limitation in plan 02 |
| Whole-run trip conditions (plan 02 AR-011) | **SHIPPED** | `app/runtime/bounds.py :: RunTripwire` adds wall clock (`RUN_WALL_CLOCK_SECONDS=600.0`, `0` disables), cumulative tokens (`RUN_MAX_TOKENS`, ships **off** pending a measured p95), repeated-tool-call detection (`LOOP_DETECTION_ENABLED` / `REPEATED_TOOL_CALL_THRESHOLD`) and a tool-failure breaker (`RUN_MAX_CONSECUTIVE_TOOL_FAILURES` in `RUN_TOOL_FAILURE_WINDOW`) to the invocation and USD bounds that already shipped. One latched reason per run, one `agent_loop.tripped` / `agentic.tripped` line naming its counted event, and a labeled partial on its own channel — `budget_halted` stays the USD cap's |
| Unbounded tool-result payload back to the model | **SHIPPED** | `TOOL_RESULT_MAX_CHARS` (default 8000, `0` disables) truncates one result's payload/error at `tool_feedback_to_history` with a visible marker, republished under `truncatedOutput` so the envelope stays parseable. Not a trip — the run continues |
| Compaction thrash (a summarizer call every turn past the threshold) | **SHIPPED** | Two watermarks: `should_compact` is the high one, `COMPACTION_TARGET_FRACTION` (0.5) the low one a pass lands on, and `COMPACTION_CUT_QUANTUM_FRACTION` (0.5) quantizes the cut over the *leading* prefix so appending a turn cannot move the boundary. Nothing persists a compaction, so this is what makes the summary cache hit across turns |
| Prompt prefix poisoned by a minute-resolution clock | **SHIPPED** | `build_system_prefix` orders memory facts → custom instructions → datetime, with the timestamp last so the stable bytes ahead of it stay cacheable under longest-common-prefix caching |
...
| Heterogeneous fallback pricing / roll-up | **SHIPPED** | Planner, workers (including a superseded sibling) and a HITL-paused worker price and attribute on the binding that served them, so the billed turn matches the orchestrator's `RunCost` subtotal with no unbilled remainder. `subtotal_usd` is the whole charge — the long-context surcharge is a disclosure slice, never an addend |
| Mid-run `run_cost` ticks | **SHIPPED** | Orchestrator emits plan / progress / final ticks with `confidence`/`phase`; every phase persists its receipt, so a reloaded meter keeps the emitted label and a summed reconstruction falls back to `estimate`/`plan` rather than claiming `exact`/`final` |
...
| Scoped per-worker tool subsets | **PARTIAL (shipped minimum)** | Workers do **not** inherit the full `advertised_tool_specs()`: `orchestrator.py` defines `_WORKER_ALLOWED_TOOLS` (`_WORKER_PROD_HITL_TOOLS` ∪ `_WORKER_FAKE_HITL_TOOLS`) and every worker call site — fan-out worker and its fallback, HITL continuation and its fallback — passes `allowed_tools=_WORKER_ALLOWED_TOOLS`, which scopes both advertise (`_tool_definitions_for` → `advertised_tool_specs(allowed_names=...)`) and `run_agent_loop` execute. What is still open is a **broader per-task** subset chosen from the plan rather than one fixed HITL-minimum allowlist |
...
| PRD 08 partial-synthesis warning chip | **SHIPPED** | Chip renders from the persisted `agentic_run_summary` live and after reload; the budget copy appears only when `budgetHalted`, so an aggregator failure gets the generic partial copy instead |
| Worker tool-HITL resume / approval idempotency | **SHIPPED** | BE-005 wait-siblings + continuation resume, pinned to the mode / tier / provider / model the pause was taken in; BE-007 claim/settle on the paused row, and a claim orphaned by a crash now reaches a durable terminal on both the pseudo-tool and registry paths without re-running a side effect |
...
- **Partial-synthesis labeling.** Resolved in favor of the chip: a budget halt appends inline prose ("answered N of M planned steps") **and** the `severity: "warning"` chip (PRD 08) renders from the persisted `agentic_run_summary`. Remaining question is copy only — how much detail the chip should carry per degrade cause.
- **Per-subagent tool subsets.** Settled in favor of scoped, least-privilege per FR-26 / SR-2: workers run on the fixed `_WORKER_ALLOWED_TOOLS` HITL minimum, not the full registry (see the PARTIAL row above). The residual question is whether each task should instead get its own subset selected from the plan — and if so, who picks it (planner output vs. an orchestrator-side policy) and how a wrong pick degrades.
...
**Partials vs this milestone's original scope (historical note — see Remaining gaps for current status):** at M3 cut, per-subagent attribution persistence and FE display were incomplete. **Current (post-M4 + hardening):** attribution **is persisted** on `SubagentPart`; FE substitution callout renders live and after reload; `execute_tool` spans are wired in `agent_loop.py`; the verifier is a **fresh-context judge** (default off; per-sample billed); mid-run `run_cost` progress ticks emit from the orchestrator and persist their receipt, so the FE labels estimates honestly on reload; heterogeneous fallback pricing prices each phase on the route that served it; fuller requested→served reason disclosure remains open; `AGENTIC_MAX_DEPTH` is config-only (depth 1 by construction).
