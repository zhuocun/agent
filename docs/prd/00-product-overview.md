...
| D11 | **DeepSeek is the main provider and the default route across the tier ladder** (`deepseek-chat` for fast/smart/auto, `deepseek-reasoner` for pro), via the OpenAI-compatible binding; PRD 02 owns the registry. Anthropic ships as an alternate backend; other Western routes remain selectable in the picker. | DeepSeek is the cost leader (token prices 30–100× below frontier) and exposes raw thinking tokens — it carries the metered free tier and the Auto default. The data-residency posture (data processed in mainland China; jurisdictional bans) is a **known, accepted tradeoff** surfaced via the `data_policy` badge, not a blocker; privacy-sensitive users can pick a Western route or use BYOK. |
