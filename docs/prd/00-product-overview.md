...
### Deferred
- **P1 (fast-follow) — several have shipped ahead of schedule (see §11 D15/D18), with truthful gating:** vision/PDF + attachments UI (*partially shipped — attach + text/PDF transcript on attachment-capable bindings (prod DeepSeek accepts none); image vision Anthropic-only*), Mermaid (*shipped*), tool/function-calling + agentic loop with HITL approval (*shipped\* — `TOOLS_ENABLED`, fake-provider v1*), web-search + live citations (*shipped\* — `SEARCH_BACKEND` default `none`, Tavily-ready; source-card list only, no inline `[n]` markers yet*), copy-on-branch (*shipped*), native slash commands (*shipped — client-static prefill*), resumable-stream replay (*shipped\* — `RESUMABLE_STREAMS_ENABLED`*). **Still deferred:** Projects/Spaces, memory transparency, TTS/dictation, customizable keyboard shortcuts, multi-format (PDF/.docx) export.
