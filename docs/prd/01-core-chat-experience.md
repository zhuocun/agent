...
- **[P1 — pin shipped; archive deferred]** Pin / archive.
  - **Shipped:** pin/unpin a conversation — pinned threads surface in a dedicated "Pinned" group in the sidebar, persisted via `patchConversation({ pinned })`. **Archive is not shipped** (only pin ships).
