...
- **FR-2b [P0]** **Registry cost-accounting schema (THIS PRD OWNS IT).** The price representation must be **structured, not a single scalar per direction**, because a flat $/token makes the cost-transparency wedge *itself* wrong on exactly the long-context, high-value turns where it matters most. The schema MUST express: **(a) tiered/threshold pricing** (e.g., a >200K-token input tier, or a per-session surcharge above a token threshold that multiplies in/out rates — these patterns exist today and are illustrative/`[verify-at-build]`, not normative); **(b) cached-input** and **(c) batch** rate multipliers; **(d) promo windows** with effective dates (a discounted rate that reverts on a date); and **(e) reasoning-token accounting** (reasoning tokens billed as output, FR-18). The cost computation (FR-36) applies this full model; where tier/promo data is missing, the value is labeled an estimate with a documented error mode rather than silently shown wrong. (Per-message **persisted** cost is owned by **PRD 04**'s data model; the **chat-surface display** is owned by **PRD 01**; this PRD owns the schema + computation.) *Acceptance: the registry can represent a threshold-tier surcharge, a cached and a batch multiplier, and a dated promo for a single model; the meter (FR-36) produces a different per-turn cost for an above-threshold vs below-threshold turn of the same model.*
...
- **FR-8 [P0]** **Auto-routing (heuristic)**: route to a Fast/cheap model for simple queries and a Smart/Pro/reasoning model for hard ones, using cheap signals (prompt length, code presence, explicit "think hard" intent, attachment presence). The routing **decision itself** (not just the resulting model) is a first-class, visible, **overridable** surface — especially when Auto routes *down* to a cheaper model — and is recorded per turn via FR-11b. (The GPT-5 "Auto router" backlash over silent downgrades is direct market validation that surfacing this is a felt differentiator; log user overrides as seed signal for a future learned router, §9 Q2.) *Acceptance: short trivia routes Fast; a long code-debugging prompt routes Smart/Pro; the routing decision is recorded and surfaced per turn and the user can override it.*
...
- **FR-16 [P0]** A **reasoning-effort toggle** ("effort/level" — e.g., off/low/medium/high) is mapped **per provider** in the registry (Anthropic adaptive / extended-thinking, OpenAI `reasoning.effort`, Gemini `thinking_level`). The user sees one normalized control; the adapter maps it. The **set of valid effort levels per provider is registry seed data verified against the live API at build, not hardcoded** (e.g., OpenAI's documented knob is low/medium/high; do not assert unverified levels such as "xhigh"). *Acceptance: the same UI control produces correct provider-specific parameters; models that don't support reasoning hide the control.*
...
| `pricing` (structured per FR-2b: base `price_in`/`price_out` per-M tokens **+ threshold tiers + cached/batch multipliers + dated promos**) | Cost meter — **not a single scalar** (nullable; from live source where possible). A scalar makes the wedge wrong on long-context turns |
...
| `data_policy` (e.g., `trains_on_data`, `data_residency`, `zdr_available`) | Per-provider data handling so the router can *enforce* "no-train routes only" defaults (PR-1) and the picker can surface a data-handling badge (privacy-first transparency feature) |
...
- **Breadth + fallback layer:** **OpenRouter** (and/or the chosen gateway) to instantly add **xAI/Grok**, Mistral, DeepSeek, Llama, etc. behind one integration, with automatic provider fallback — a rich picker on day one without N integrations. (OpenRouter now offers ~1M free BYOK requests/month then a ~5% fee — [verify-at-build]; relevant to BYOK-at-launch economics, FR-6 / PRD 05.)
- **Include xAI/Grok in the lineup at MVP (registry entry, via the breadth/gateway layer):** "every major model in one place" (PRD 00 §1) cannot credibly omit a major provider. Grok is a cheap-frontier option — flagship-adjacent quality at near-Flash output pricing (e.g., Grok 4.3 ≈ $1.25 in / $2.50 out per 1M, 1M context; plus a budget Grok 4.1 Fast tier — illustrative/`[verify-at-build]`) — a natural candidate for the "Smart" tier. Direct integration is a P1 reassessment; **gate Grok from being a *default* route until its data-handling posture is reviewed** against PR-1 (same bar applied to DeepSeek). *(Flag for the PRD 05 roadmap worker: Grok registry entry = MVP/P0.)*
- **Defer as *direct* integrations:** xAI / DeepSeek / Mistral / Llama — obtain via the breadth layer initially.
- **Direct-vs-aggregator tradeoff:** direct = best features/latency/SLA/data-control; aggregator = breadth + fallback fast, thin fee, less control. **Recommended: both** (direct for primaries, aggregator for breadth/fallback). Final gateway choice coordinated with **PRD 04 §5** (Vercel AI Gateway vs OpenRouter vs LiteLLM) — architecture's current lean is **Vercel AI Gateway as the MVP default**, with OpenRouter as the breadth/fallback layer (not necessarily the default route). **Gateway-native guardrails (PII redaction, jailbreak detection, moderation hooks) are an explicit selection criterion** — see §6.1 SR-4.
- **Privacy constraint on provider choice:** prefer routes/settings with **no training on user data by default** (enforced via the registry `data_policy` field, §5.2 / PR-1); note data-residency/governance concerns for some providers (e.g., DeepSeek, and review Grok's posture) when selecting defaults (§6).
- **Free-tier default model (THIS PRD OWNS the selection):** the always-available default for free/guest users MUST be a **Western-hosted, no-train-by-default route** — pick from **Gemini Flash / an OpenAI mini / Claude Haiku / a Mistral-EU model** (final pick coordinated with PRD 05 cost gating; all model names illustrative/`[verify-at-build]`). **Do NOT default to the DeepSeek-hosted API**: government/enterprise bans plus an Italy consumer block and data-stored-in-China posture make it a poor privacy-first default. Western-hosted DeepSeek **open weights** remain a **[P2+]** option (a separate, self-/Western-hosted line, not the DeepSeek-hosted API). *(PRD 05 references this rationale for its tier/cost gating.)*
...
### 6.2 Privacy — **[P0]**
- **PR-1 [P0]** **No training on user chats by default.** Choose provider routes/settings that honor this; document per-provider data handling in the registry/config **via the `data_policy` field (§5.2)** so the router can *enforce* "default to no-train routes only" and the picker can surface a data-handling badge. This constrains which providers/routes can be defaults — including the free-tier default (§5.3) and the gating of DeepSeek-hosted/Grok routes. Note that some providers' **free tiers may use content to improve products while the paid tier does not** — exactly the per-route distinction `data_policy` must capture ([verify-at-build]).
...
9. **Free-tier default model — which Western no-train route?** *Narrowed:* the **DeepSeek-hosted API is dropped** as a candidate (government/enterprise bans, Italy consumer block, data-in-China — §5.3); the remaining choice is among **Gemini Flash / an OpenAI mini / Claude Haiku / a Mistral-EU model**, decided on the `data_policy` no-train guarantee plus cost gating (coordinated with PRD 05). Western-hosted DeepSeek **open weights** stay a separate **[P2+]** line, not a default. Open sub-question: which specific provider routes/settings actually guarantee no-training-by-default at the free tier? (PR-1, §5.3)
...
> - **All pricing** ($/M tokens) — re-priced and promo'd frequently; **structured per FR-2b** (threshold tiers, cached/batch multipliers, dated promos), not a scalar.
...
> - **Per-provider data handling** (`data_policy`: trains-on-data / residency / ZDR) — varies by route and tier (PR-1, §5.2 / G7).
