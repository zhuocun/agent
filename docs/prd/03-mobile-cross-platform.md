...
- **[P1] Dictation (speech-to-text) in the composer (B1).** A **mic button** adjacent to Send starts/stops capture; this promotes the former "progressive-enhancement note" to a **specified feature** with a mandatory editable-transcript UX and transparency framing. The STT **capability/registry/cost/`data_policy` contract is owned by PRD 02 §4.8 FR-33a (B3 spine)**; this section owns the **mobile composer UX**.
  - **Two-track, capability-detected:** **Track A — Web Speech API (`SpeechRecognition`)** as progressive enhancement, streaming **interim (partial) results** into the textarea live (the zero-infra default where supported); **Track B — server-side / BYOK STT** (audio captured via `MediaRecorder`, posted to a BE STT route per PRD 02 FR-1) — the **privacy-aligned path** and the **only** path that works in an installed iOS PWA (Track A is broken there).
  - **Editable transcript before send is mandatory — dictation never auto-sends.** Recognized text lands in the normal composer textarea where the user edits/appends/discards it, then sends with the existing Send control (button / Enter per the §4.3 platform rules). (STT is lossy; an auto-send loop is an accessibility + trust failure.)
  - **Privacy correction (verified):** Web Speech recognition is **NOT on-device** — on iOS Safari it **sends audio to Apple** (explicit "send audio to Apple" permission modal) and Chrome is also typically server-side. For a privacy-first product (PRD 00 §7), this must be a **disclosed, optional** feature: surface an in-UI disclosure ("dictation sends audio to your browser vendor") before first use — treat the disclosure as **P0-for-the-feature**, naming the actual route's data posture from the registry `data_policy` (PRD 02 B3). Do **not** describe it as on-device/privacy-preserving.
  ...
  - Caveats surfaced in UI: unsupported in Firefox; **does not work in an installed iOS PWA** (works in Safari tab). **Hide/disable the mic where no STT engine is available** (no silently-dead control) unless a server STT (Track B) route is configured; in an installed iOS PWA with Track A unavailable, route to Track B or hide the mic.
  - **Recording-state affordance:** a clear "listening" indicator with an elapsed timer and a visible Stop; honors `prefers-reduced-motion` (static indicator). Mic tap target ≥44–48px (§4.8); the listening state is announced **once** via the existing polite status region (`live-region.tsx`), not spammed per partial.
  - *Acceptance:* tapping the mic with a supported engine streams interim transcript into the composer, fully editable, and never auto-sends; the first capture in a session is gated by a disclosure naming the transcription route and its data posture (declining cancels capture, leaving the composer untouched); on a context with no STT engine the mic is absent/disabled with an accessible explanation, never appearing then failing on tap. (Cite D22.)
- **[P2] Hands-free voice mode (B4).** A continuous **speak → model answers → spoken reply** loop layered on B1 (STT Track B) + read-aloud (§4.8 / PRD 02 FR-33b TTS), opening a focused voice surface with a waveform/state indicator and an always-visible **End** control. **Barge-in** (speaking over playback interrupts within ~1s and starts a new capture) is the voice-mode Stop and the hardest mobile detail. **Hard guardrail: conversational chat by voice, NOT a task-running voice agent** — no tool side effects by voice; an approval-gated tool (`TOOLS_ENABLED`) never auto-approves and pauses at the visual `awaiting_approval` gate. **Transparency preserved in-voice:** a glanceable served-model/tier chip every turn, and any substitution is reflected + written to the persisted turn (PRD 02 FR-11b / FR-33c) — no silent downgrade even in voice. **Top mobile risk = iOS audio-session + autoplay** (a user gesture starts the session; background-tab capture is restricted); honors reduced-motion (calm waveform) and holds the `Screen Wake Lock` (§4.9) during the loop, releasing on End. The full loop spec + realtime-infra flag are PRD 02 FR-33c. (Cite D22.)
...
- **[P1] Read-aloud (text-to-speech) on mobile (B2) — flagship a11y feature.** A labeled "Read aloud" control on assistant messages (and an opt-in, off-by-default auto-read) lets users consume long answers hands-free / eyes-free (mobile multitasking, low-vision, dyslexia, reading fatigue). The behavior/registry/cost contract is **PRD 02 §4.8 FR-33b** (two-track: Web `speechSynthesis` free OS voices vs server/BYOK neural TTS); this section owns the **mobile UX**: mobile **autoplay-audio policy requires a user gesture for first playback** (auto-read's first turn may need a one-time "enable audio" tap); read-aloud controls are keyboard-operable + screen-reader-labeled and must **not hijack the screen reader's own speech** (playback is user-initiated or explicitly opted-in); a **reading-position highlight** (current sentence, caption-style) honors `prefers-reduced-motion` (no scroll-jank). `Screen Wake Lock` (§4.9) should hold during long read-aloud so audio isn't cut, releasing on stop. (Cite D22.)
- **[P1]** **Dynamic type:** respect iOS Dynamic Type / Android `sp`; use `rem`, avoid fixed-px text.
- **[P1] Reduced motion (extended to media/voice chrome):** honor `prefers-reduced-motion` for streaming animations and transitions **and** for **every new animated media/voice surface** — the B1 listening indicator (§4.7), the B4 voice-mode waveform (§4.7), image-gen reveals (PRD 02 FR-33d), and read-aloud caption highlights — all degrade to static. (Cite D22.)
...
- **[P1]** **`Screen Wake Lock`** during long streaming answers so the screen doesn't dim mid-response (available for home-screen web apps since Safari 18.4). Honor battery/system constraints; release on stream end. **Also hold the wake lock during long read-aloud playback (B2, §4.8) and throughout a voice-mode loop (B4, §4.7)** so audio/the session isn't cut mid-response; release on stop / End. (Cite D22.)
- **[P1] Microphone permission & capture (B1/B4 dictation + voice mode).** Mic capture (`getUserMedia` / `MediaRecorder` for the Track-B STT path, or the Web Speech permission prompt for Track A) requires a **user gesture** and an OS/browser permission grant; request it **contextually on first mic tap**, not on load, and **degrade silently** where denied or unsupported (hide/disable the mic — §4.7). **Web Speech (Track A) is broken in an installed iOS PWA** (works in a Safari tab) — fall back to a server STT route (Track B) or hide the affordance there. Audio bytes are **transient** (never persisted — PRD 02 FR-33a). (Cite D22.)
- **[P0]** **Enumerate & design around iOS PWA limits** (see §6.4 table). Specifically:
  ...
  - Storage is **disk-proportional (tens of GB)**, **not ~50 MB** (corrected §4.6); real constraint is **7-day eviction unless `navigator.storage.persist()` is granted**; **no background sync/fetch**; **no auto-install prompt**; **Web Speech (dictation Track A) broken in installed iOS PWA** → fall back to a server/BYOK STT route (Track B) or hide the mic (B1, §4.7).
...
| Web Speech (recognition) — dictation Track A (B1, §4.7) | Supported (Chromium) | **Broken in installed PWA** (works in Safari tab) → fall back to server/BYOK STT (Track B) or hide the mic |
