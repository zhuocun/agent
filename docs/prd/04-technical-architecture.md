...
- **[P2 — spec'd; not built] Agentic mode (orchestrated bounded subagents):** the multi-agent extension of the shipped agent loop (PRD 02 §4.6 FR-26c–FR-26k; PRD 00 §11 D33–D40; build plan `docs/plans/01-agentic-mode.md`). A thin **orchestrator** at the existing `streaming/handler.py::_build_provider_iter()` seam fans a single in-turn assistant turn out to **N bounded subagents — each a reuse of `run_agent_loop`** over a scoped sub-prompt — multiplexes their `ProviderEvent` streams (subagent-tagged) back onto the one SSE turn, and aggregates one answer. **No new transport, no new loop engine, no new infra:** workers run as in-turn `asyncio` tasks (bounded by a concurrency semaphore) on the same request task so the shipped cancellation / persistence / fallback paths are unchanged; the per-run cost reads the shipped `providers/pricing.py` (run total = sum of subagent costs); plan approval reuses the `awaiting_approval` terminal + `toolApproval` resume; subagent-scoped parts are additive to the typed `message.parts` union (no column-type migration); OTel `invoke_agent`/`execute_tool` spans ride the shipped env-gated tracing path. Default-OFF behind `AGENTIC_ENABLED` (gated by `TOOLS_ENABLED`), fake-provider-first; **real-provider subagent wiring is the gating prereq**, and per-run USD cap + fan-out/recursion bounds are config (no hardcoding). Background/scheduled/out-of-turn execution stays out of scope (the D23/D33 chat-anchored guardrail).
