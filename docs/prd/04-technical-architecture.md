...
- **Current implementation (MVP deploy):** the deployed BE binds the OpenAI-compatible adapter to the DeepSeek-hosted API. Config today: `PROVIDER_BACKEND=openai`, `OPENAI_BASE_URL=https://api.deepseek.com`, and per-tier overrides `OPENAI_MODEL_FAST` / `OPENAI_MODEL_SMART` / `OPENAI_MODEL_AUTO` = `deepseek-chat` with `OPENAI_MODEL_PRO=deepseek-reasoner` (env-var names match `api/app/config.py` and `api/.env.example`). This is a pragmatic cost choice for the MVP deploy and deviates from the non-DeepSeek-hosted Western no-train default the PRDs target — see **PRD 00 §11 D11** for the strategic discussion and PRD 02 §5.3 for the model-selection rule. This bullet just captures the current production binding, not a recommendation.
