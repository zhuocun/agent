...
- **[P2 — spec'd; not built] Agentic mode (orchestrated bounded subagents):** the multi-agent extension of the shipped agent loop (PRD 02 §4.6 FR-26c–FR-26k; PRD 00 §11 D33–D40; build plan `docs/plans/01-agentic-mode.md`). A thin **orchestrator** at the existing `streaming/handler.py::_build_provider_iter()` seam fans a single in-turn assistant turn out to **N bounded subagents — each a reuse of `run_agent_loop`** over a scoped sub-prompt — multiplexes their `ProviderEvent` streams (subagent-tagged) back onto the one SSE turn, and aggregates one answer. **No new transport, no new loop engine, no new infra:** workers run as in-turn `asyncio` tasks (bounded by a concurrency semaphore) on the same request task. The handler's **transport contract** (one `ProviderEvent` iterator in, one SSE turn out) is unchanged; **accumulation** (`_apply_event` / `_build_parts`) becomes subagent-aware additively (un-tagged streams still fold into one default group). Turn-level provider fallback stays per-turn (not per-subagent); per-worker degrade is orchestrator logic. Cancellation must explicitly tear down in-flight worker tasks (especially under resumable detached producers, where disconnect ≠ cancel). The per-run cost reads the shipped `providers/pricing.py` (run total = sum of subagent costs; heterogeneous breakdowns persist on subagent parts); plan approval emits a `terminal` with `status:"awaiting_approval"` + estimate attribution, resumed via `toolApproval`; subagent-scoped parts are additive to the typed `message.parts` union (no column-type migration); **new** OTel `invoke_agent`/`execute_tool` spans (manual instrumentation — `tracing.py` today only auto-instruments FastAPI + SQLAlchemy) ride the shipped env-gated tracing path. Default-OFF behind `AGENTIC_ENABLED` (gated by `TOOLS_ENABLED`), fake-provider-first; **real-provider subagent wiring is the gating prereq**; per-run USD cap is admission-controlled + fan-out/recursion bounds are config (no hardcoding). `deep_research` is Pro/BYOK-gated. Background/scheduled/out-of-turn execution stays out of scope (the D23/D33 chat-anchored guardrail).
