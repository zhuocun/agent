...
### 4.5 Feature-expansion backlog (this brainstorm — D19–D32 + the D33–D40 agentic pass; D19/D20/D21/D22/D23/D24/D27/D30 shipped, D31 partial, D33–D40 agentic shipped\* behind flag with M4 partial, rest specced)

The feature-expansion pass (PRD 00 §11 **D19–D32**, extended by the **D33–D40 agentic-mode pass**) specced a set of net-new, on-strategy features — each deepening one of the six wedges (multi-model · transparency · cost · privacy · mobile-web · accessibility). **Several have since shipped on `main`: D24 (inline `[n]` citations + contract, #143), D27 (spend analytics + budget guardrails + value-aware picker, #144), D30 (trust surfaces, #145), D19 (transparent long-term memory, #148), D22 (dictation + read-aloud, #149), D31 (granular per-conversation retention + scheduled purge — partial; ephemeral/incognito + multi-format export remain, #147), D20 + D23 (Projects/Spaces, prompt library, customizable keyboard shortcuts, #151), the D20-tail + D21 "organize + find" wave (Conversation org v2 — tags/archive/bulk actions — plus advanced history search with transparency-native filters + query-time Postgres FTS, #153), and **D33–D40 (agentic mode — shipped\* behind `AGENTIC_ENABLED`, M4 partial)** — marked **Shipped** / **Partially shipped** in the table below; the rest remain specced (not built). The table is the roadmap placement; **full specs live in the workstream PRDs** (01/02/03/04/06/07/08). It expands several summary rows in §4.3 — the Projects/Spaces + memory-transparency row, the slash-commands/prompt-library + customizable-shortcuts row, the richer-usage-credit row, the web-search "no inline `[n]` markers yet" note (now closed by D24), and the P2 Artifacts/Canvas/RAG/voice/image-gen/MCP/teams cluster row — into per-feature detail. Same P0/P1/P2 legend; **P2\*** = P2 stretch/later.
...
| Agentic mode (in-turn orchestrator over bounded subagents; single-loop default + opt-in Deep Research) | **P2 — SHIPPED\* behind `AGENTIC_ENABLED` (M4 partial)** | 00 D33–D40, 02 §4.6 FR-26c–FR-26k, plans/01-agentic-mode.md, 04 §5.1, 07, 08 | M0–M3 on `main` (fake-provider proven: `api/tests/test_agentic_*.py`, `web/tests/e2e/agentic.spec.ts`). Per-run USD cap + plan-approval HITL + subagent panel + live cost meter shipped. **M4 open:** per-worker fallback, per-subagent attribution UI, real-provider E2E. Pro/BYOK-gated `deep_research`. Do not enable in real-key prod until M4 proven. |
