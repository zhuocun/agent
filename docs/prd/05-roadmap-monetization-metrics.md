...
### 4.5 Feature-expansion backlog (this brainstorm — D19–D32 + the D33–D40 agentic pass; D19/D20/D21/D22/D23/D24/D27/D30 shipped, D31 partial, rest specced)
...
| Agentic mode (in-turn orchestrator over bounded subagents; single-loop default + opt-in Deep Research) | **P2** | 00 D33–D40, 02 §4.6 FR-26c–FR-26k, plans/01-agentic-mode.md, 04 §5.1, 07, 08 | Chat-anchored, default-OFF `AGENTIC_ENABLED` (gated by `TOOLS_ENABLED`), fake-provider-first; reuses the shipped `run_agent_loop` at the `_build_provider_iter()` seam; per-run USD cap + fan-out/recursion bounds + plan-approval HITL + subagent-scoped transparency/OTel; real-provider wiring is the gating prereq. Stacks reasoning (~4–15×) and multi-agent (~15×) cost multipliers — **a high-cost mode gated to Pro/BYOK** (the per-run fan-out cost is uneconomic on the metered free tier). |
