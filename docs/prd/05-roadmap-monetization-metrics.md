...
> **MVP (P0):** A fast, polished, mobile-web-first responsive chat app (Next.js 16 + AI SDK v6 over SSE, Better Auth, PWA layer) that nails streaming + streaming-safe markdown rendering, composer ergonomics, conversation management, core message actions, a collapsible reasoning panel, onboarding, basic settings, share/export, a command palette, and an accessibility baseline — delivered over a **multi-provider model picker with a visible "model used + token cost" transparency surface** (the core wedge, cheap via the provider-abstraction layer). It launches with a **no-train-by-default privacy posture**, **AI-interaction disclosure**, a **metered free tier** plus a **Pro subscription with explicit metered caps + transparent USD credit overage** (exact Pro price an open question — see §9) and a **BYOK option (no token markup)**, governed by aggressive model routing to protect margin. It targets power users/developers; everything heavier (deep comparison, artifacts, search/RAG, memory, voice, image-gen, MCP, native, enterprise) is explicitly later.
...
- **US-state-law AI-interaction disclosure** (CA SB 243 / CO SB 205, live now) is a **P0 launch-gate for the US market**, sitting next to the EU AI Act note below — not EU-only. Our P0 disclosure covers the core requirement (see §7.5).
- **EU AI Act content-marking** (machine-readable AI-content labels) should be designed alongside any **image/media generation** (P2) and is currently treated as due **Dec 2, 2026** `[VERIFY]`; **AI-interaction disclosure** is P0 (due ~Aug 2026 `[VERIFY]`). **Unresolved conflict needing legal sign-off:** PRD 04's review argues Article 50(2) content-marking may *also* bind **2 Aug 2026** (which would pull watermarking into P0 for an EU launch), and a **May 2026 provisional agreement** is reshuffling Act deadlines — keep both `[VERIFY]` flags (see §7.5).
...
| **T3 Chat** (**anchor-to-beat**) | trial | **$8/mo Pro = 1,500 msgs/mo** (Claude capped 100/mo; +$8/100 Claude credits) | — | — | API-rate arbitrage; transparent-about-limits, no privacy story |
...
**Read:** the legacy consumer anchor is ~$20/mo "Plus/Pro"; prosumer tiers ($100/$200) are **usage multipliers, not better models**. But the sharper near-term anchor is **T3 Chat at $8/mo** (multi-model, message-metered, transparent caps) — the closest competitor to our exact pitch and the **explicit price/feature anchor-to-beat**. Our differentiation over T3 is privacy/no-train + accessibility/mobile-web polish + genuine cost transparency (T3 leans on opaque credit redenomination and has no privacy story). We differentiate on transparency, model choice, and BYOK rather than on a higher price; the **exact Pro price (~$15–20 vs the $8 floor) stays an open question** (§9.1).
...
- **[MVP]** **CA SB 243 (Companion Chatbot Act)** — mandatory "you're talking to an AI" disclosure, crisis-handling protocols, minor protections — **effective Jan 1, 2026.** Our P0 disclosure covers the core disclosure requirement.
...
- **[MVP]** **CO SB 205 (high-risk AI; consumer AI-interaction disclosure)** — **enforcement Jun 30, 2026** (postponed from Feb 1). Core disclosure covered by P0.
...
**Minors / companion-persona decision — `[OPEN QUESTION, see §9.11]`:** CA SB 243 + surviving child-safety preemption carve-outs mean we must explicitly decide **whether we serve minors / offer companion-style personas.** If yes, additional obligations attach (crisis protocols, break reminders, stronger minor protections). **Contested — flag, do not decide here.**

**EU AI Act layer (CONTESTED dates — keep `[VERIFY]`, do not downgrade):**
- **[MVP]** **AI-interaction disclosure** — clearly tell users they are interacting with an AI (EU AI Act Article 50 transparency, **effective ~Aug 2, 2026** `[VERIFY]`). Penalties up to **€35M or 7%** of worldwide turnover. `[VERIFY]`
- **[MVP]** Baseline safety filtering / abuse monitoring (see §7.4).
- **[Later/contested — `[VERIFY]`]** **Machine-readable marking/labeling of AI-generated content** (deepfakes, public-interest text) — currently treated as deferred to **~Dec 2, 2026** and to be designed alongside any image/media generation (P2). **Two unresolved conflicts needing legal sign-off:** (a) a **May 7, 2026 Council/Parliament provisional agreement** is reshuffling Act deadlines (all dates now provisional pending Official Journal publication); and (b) **PRD 04's review argues Article 50(2) content-marking may also bind 2 Aug 2026 (not Dec 2)** — which would pull watermarking into P0 for an EU launch. Keep both `[VERIFY]` flags until legal confirms.
...
| **Compliance timeline slip** (EU AI Act Aug/Dec 2026; contested content-marking date) | Medium / High | AI-interaction disclosure in P0; design content-marking alongside media-gen; track official EC pages `[VERIFY]`; **resolve Aug-2-vs-Dec-2 content-marking conflict + May-2026 provisional amendment with legal** before EU launch (§7.5). |
| **US regulatory-surface risk** (state AI-law patchwork + shifting federal preemption, beyond EU) | Medium / High | CA SB 243 / AB 2013 live now, CO SB 205 enforces Jun 30 2026; preemption contested and carves out child-safety. P0 disclosure covers the core; track state patchwork; resolve the minors/companion-persona gate (§7.5/§9.11). |
...
1. **Pro price point `[CONTESTED — flag, don't decide]`:** ~$15–20 vs T3 Chat's **$8** anchor? Our ~$15–20 is **2–2.5× T3's price for a similar multi-model surface** — justify the premium (privacy + a11y + mobile-web + true cost transparency) or reprice. Trade conversion volume against margin and both the ~$20 incumbent and $8 aggregator anchors. `[VERIFY]` market.
...
6. **EU launch timing vs AI Act dates `[CONTESTED — flag, don't decide]`:** do we gate EU availability on the Aug 2 2026 transparency obligation, or launch globally with disclosure built in? Dates are **contested** — a May-2026 provisional agreement is reshuffling deadlines and PRD 04 argues content-marking may bind 2 Aug 2026 (not Dec 2). Needs legal sign-off. `[VERIFY]` dates (§7.5).
...
11. **Minors / companion-persona decision `[CONTESTED — flag, don't decide]`:** do we serve minors or offer companion-style personas? Triggers CA SB 243 + surviving child-safety preemption carve-outs (crisis protocols, break reminders, minor protections). Needs explicit product + legal decision (§7.5).
...
- **EU AI Act dates** (transparency ~Aug 2 2026; content-marking ~Dec 2 2026) and **penalty figures (€35M/7%)** — **CONTESTED:** May-2026 provisional agreement reshuffles deadlines (provisional pending Official Journal); PRD 04 argues content-marking may bind 2 Aug 2026. Legal sign-off required (§7.5/§9.6).
- **US state AI laws** — CA SB 243 + AB 2013 (live 1/1/26), CO SB 205 (enforce 6/30/26), Dec-2025 federal preemption EO (child-safety carve-outs survive; not self-executing). Confirm scope before locking compliance + the minors gate (§7.5/§9.11).
...
- **T3 Chat $8/mo (1,500 msgs; Claude capped 100/mo)** — the price/feature anchor-to-beat; re-confirm tiers before external use.
