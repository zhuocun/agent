...
**Shipped (as-built):** turn-level `cost_usd` roll-up; per-subagent `cost_usd` on `SubagentPart` markers; live `run_cost` meter during streaming (`SubagentPanel`). Pre-spawn admission + mid-flight budget kill (`budget.py`, `orchestrator.py`). Per-subagent `ModelAttribution` + substitution + heterogeneous `cost_breakdown` are now persisted on `SubagentPart` — `_build_agentic_parts` populates each worker's attribution via `build_attribution` (`handler.py`; `test_agentic_resilience.py::test_subagent_parts_carry_attribution`).

**Not shipped:** the per-subagent served-model/substitution **callout in the FE** — `subagent-panel.tsx` still renders the turn-level `AttributionRow` only, so the per-subagent attribution is persisted but not yet displayed per worker.
