...
> **Agentic-run failure states (P2 — SHIPPED\* behind `AGENTIC_ENABLED`, gated by `TOOLS_ENABLED`; per-worker degrade pending M4).** **Shipped:** a per-run budget admission stop or mid-flight cap breach degrades gracefully to a **labeled partial synthesis** (inline answer text: "answered N of M planned steps") rather than an opaque `error`; plan approval reuses the `awaiting_approval` terminal + `toolApproval` resume. **Not shipped (M4):** when a fan-out subagent fails on provider 5xx/timeout, the worker should degrade **not the whole run** — mark the subagent failed and synthesize from surviving workers with a `severity: "warning"` callout ("Answered from N of M sub-tasks; one couldn't finish"). Today a genuine worker exception **re-raises and fails the whole run** (`orchestrator.py`). **All-workers-fail** as a hard `PROVIDER_ERROR` terminal (vs persisting an empty answer) is not clearly implemented. Untrusted-output discipline holds across the tree (D39); per-subagent attribution + the per-run cost roll-up follow PRD 07 §4.4. (Cite D37/D39; PRD 02 §4.6 FR-26g, plans/01-agentic-mode.md.)
...
| Agentic-run errors (per-run budget halt → graceful partial synthesis; plan-approval pause) | — | — | **P2 — SHIPPED\* (`AGENTIC_ENABLED`, gated by `TOOLS_ENABLED`)** for budget-halt partial synthesis + plan-approval pause; **per-worker provider failure degrade pending M4** (PRD 02 §4.6 FR-26g, plans/01-agentic-mode.md) |
