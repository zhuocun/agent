...
> **Agentic-run failure states (P2 — SHIPPED\* behind `AGENTIC_ENABLED`, gated by `TOOLS_ENABLED`; warning-chip surfacing pending M4).** **Shipped:** a per-run budget admission stop or mid-flight cap breach degrades gracefully to a **labeled partial synthesis** (inline answer text: "answered N of M planned steps") rather than an opaque `error`; plan approval reuses the `awaiting_approval` terminal + `toolApproval` resume. **Per-worker degrade + fallback are now shipped (M4):** a fan-out subagent that fails on a provider error degrades **that worker only** — it is marked failed and the run synthesizes from surviving workers, ending `done` with an inline "sub-agent(s) failed and were omitted" callout (`orchestrator.py`; `test_agentic_resilience.py::test_one_worker_failure_degrades_to_partial`); a retryable worker error falls back to the secondary route tagged `substitution` (`::test_one_worker_retryable_falls_back`). **All-workers-fail** degrades to a graceful `done` synthesis noting "no worker findings" rather than a hard `PROVIDER_ERROR` terminal (`::test_all_workers_fail_still_done`) — a deliberate never-opaque-error choice. **Still not shipped (M4):** the dedicated `severity: "warning"` callout chip ("Answered from N of M sub-tasks; one couldn't finish") — the degrade is surfaced as inline prose only. Untrusted-output discipline holds across the tree (D39); per-subagent attribution + the per-run cost roll-up follow PRD 07 §4.4. (Cite D37/D39; PRD 02 §4.6 FR-26g, plans/01-agentic-mode.md.)
...
| Agentic-run errors (per-run budget halt → graceful partial synthesis; plan-approval pause; per-worker degrade/fallback) | — | — | **P2 — SHIPPED\* (`AGENTIC_ENABLED`, gated by `TOOLS_ENABLED`)** for budget-halt partial synthesis, plan-approval pause, and per-worker provider failure degrade + retryable fallback (`test_agentic_resilience.py`); **dedicated `warning`-chip surfacing still pending M4** (PRD 02 §4.6 FR-26g, plans/01-agentic-mode.md) |
...
> - **Agentic-run budget halt → P2 Shipped\* (`AGENTIC_ENABLED`):** pre-spawn admission stop and mid-flight cap breach produce a labeled partial synthesis (not an `error`); plan approval reuses `awaiting_approval` + `toolApproval`. **Per-worker provider-failure degrade + retryable fallback are now shipped (M4)** — see the agentic callout in §5.4; only the dedicated `severity: "warning"` chip surfacing remains pending.
