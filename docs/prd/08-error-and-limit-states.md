...
> - **Agentic-run budget halt → P2 Shipped\* (`AGENTIC_ENABLED`):** pre-spawn admission stop and mid-flight cap breach produce a labeled partial synthesis (not an `error`); plan approval reuses `awaiting_approval` + `toolApproval`. Per-worker provider-failure degrade is **not** shipped — see the agentic callout in §5.4.
