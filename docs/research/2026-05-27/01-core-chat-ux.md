...
- **Accessibility is the most defensible wedge and the research strongly validates it.** A March 2026 screen-reader teardown of Claude.ai documents concrete gaps: no completion announcement (Claude's response content is *not* in a live region, so nothing is announced at all), an inaccessible keyboard-shortcuts dialog, and sidebar landmark gaps. Separately — from the article's comments + general a11y guidance, *not* the teardown's own Claude findings — the naive fix of wrapping the streaming text node in `aria-live="polite"` is a known anti-pattern that causes NVDA/JAWS to re-read partial text token-by-token. The PRD should add the shortcuts-dialog and sidebar-landmark gaps as explicit acceptance criteria and specify a success-path completion announcement (currently unspecced).
...
- **March 2026 Claude.ai screen-reader teardown documents concrete gaps:** (1) **no announcement when generation completes** — Claude's response content is *not* placed in a live region, so the screen reader never notifies the user (they manually re-check every turn); (2) **Keyboard Shortcuts dialog is fully inaccessible** (focus never enters; flagged a "Blocker"); (3) **sidebar lacks landmarks**, forcing top-of-page traversal; (4) missing shortcuts for input focus / new chat / delete chat. *Separately — from the article's comments (Anouk Radix) + general a11y guidance, not the teardown's own findings:* wrapping the streaming text node in `aria-live="polite"` causes NVDA/JAWS to **re-read partial text token-by-token**. (Source: dev.to/wiscer screen-reader-experience-analysis-on-claudeai, 2026-03-04 — 2026-05-27.)
  - *Implication for us:* This is gold for the a11y wedge. Note the two distinct failure modes: the teardown's documented Claude bug is *nothing announced* (no live region), while the naive `aria-live="polite"` fix in PRD §5.7 risks the *opposite* anti-pattern (token-by-token re-reading). **Recommended announce model: do NOT put the streaming text node itself in an aria-live region; announce discrete status transitions ("Generating", "Response ready") via a separate polite status region, and make the completed message body navigable but not auto-announced.** Add the inaccessible-shortcuts-dialog and sidebar-landmark gaps as explicit P0 acceptance criteria — they are cheap and directly beat the measured leader.
...
|---|---|---|---|---|
| 1 | [risk] | 01 §5.7; 08 §9; 06 §3.4 | **Wrapping the streamed answer node in `aria-live="polite"` is a known anti-pattern that causes NVDA/JAWS to re-read partial text token-by-token** (from the teardown's comments + general a11y guidance; the teardown's *own* Claude finding is the inverse — content is *not* in a live region, so nothing is announced). PRD 01 §5.7 says the streaming answer "is announced appropriately (e.g., `aria-live="polite"`)", which invites the anti-pattern. PRD 08 §9's "announce once" is currently scoped to *errors* only — the success-path completion announcement ("Response ready") is **unspecced in both PRDs**. | Reconcile: the **streamed text node must NOT be a live region.** Announce discrete status transitions via a separate polite status region; the completed body is navigable, announced once. Add the success-path completion-announcement requirement to PRD 08 §9 (or PRD 06 §3.4 live-region spec), and make PRD 01 §5.7 reference it + state the anti-pattern explicitly. |
...
1. **Fix the streaming a11y announce model (Finding 1).** Do not make the streamed text node a live region. Status-only polite region + announce-once-on-complete (spec the success-path announcement, which both PRDs currently omit). This is the difference between owning the a11y wedge and shipping a known streaming-a11y bug — either Claude's silent no-announcement or the opposite token-by-token re-reading.
