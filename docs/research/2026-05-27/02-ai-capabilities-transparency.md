...
- **Privacy posture validates the PRD's gating decisions.** Western API defaults (OpenAI, Anthropic, Google paid) are **no-train-by-default**; Anthropic dropped API retention to **7 days**. By contrast **xAI Grok trains on consumer data by default (opt-out required)** and **DeepSeek-hosted stores data in mainland China** and is banned/blocked in Italy, US agencies, South Korea, Australia, Taiwan, India. PRD 02's `default_route_eligible:false` + `data_policy_review_status:pending` gate for Grok and DeepSeek is well-justified.
...
| **Anthropic Claude Haiku 4.5** | 1M | $1 | $5 | 90% off | fast tier; **free-tier default candidate** | No | 7-day | aipricing.guru/anthropic-pricing |
...
| **Google Gemini 3.5 Flash** | ~1M | $1.50 | $9 | $0.15 | flat pricing (no threshold); **free-tier default candidate** | No | Ent. | ai.google.dev/gemini-api/docs/pricing |
...
| **DeepSeek V4-Pro** | 1M | $0.435 | $0.870 | — | exposes raw thinking tokens; **promo reverts 2026-05-31 → ~4× higher** | **Data processed/stored in mainland China; banned/blocked in several jurisdictions** | No | api-docs.deepseek.com/quick_start/pricing; infoworld.com |
...
| **Mistral Small 3.1** | ~128K | $0.20 | $0.60 | — | budget EU tier; **free-tier default candidate** | No | EU | devtk.ai/mistral-api-pricing-guide-2026 |
...
- **Implication for us:** PRD 02 FR-2b/§5.2 and PRD 07 §4.1 must represent **(a)** a threshold + whole-session multiplier (OpenAI), **(b)** stepped per-band base rates (Gemini), and **(c)** "no surcharge" as a first-class, surfaceable fact. The current single `threshold_tier` multiplier handles (a) but is awkward for (b). See §4 finding T1.
...
- **Visibility differs:** OpenAI returns *summaries* (`summary:auto`); Anthropic returns *condensed* thinking and **omits thinking by default on Opus 4.7**; Gemini hides raw chain but inflates output token counts; **DeepSeek V4-Flash is the only major model exposing raw thinking tokens.** (help.apiyi.com/gemini-3-1-pro-thinking-tokens; tokenmix.ai — 2026)
...
### Theme E — Privacy posture: Western paid APIs no-train by default; Grok & DeepSeek are the outliers
...
- **DeepSeek**: data processed/stored in **mainland China**; banned/blocked in **Italy (GDPR), US agencies, South Korea, Australia, Taiwan, India**; a Wiz-discovered open database leaked 1M+ records. (tomsguide.com/deepseek-ai-banned-italy; aitechtonic.com/deepseek-ban — 2026)
- **Implication for us:** Strongly validates PRD 02 §5.3 / D11 / PR-1. The free-tier default should be **Gemini 3.5 Flash, an OpenAI mini, Claude Haiku 4.5, or Mistral Small 3.1** — all no-train-by-default. **Grok must keep `default_route_eligible:false` until its *API* (not consumer) data posture is confirmed** — note the consumer default-train finding is about X posts, and the *API* posture is a separate, still-open question worth a dedicated registry note.
...
- **A7 [inconsistency] — FR-8/§5.3 gate Grok and "DeepSeek-hosted routes" from Auto, but Grok's exclusion rationale conflates consumer-train with API posture.** The decision to gate is correct; the *stated reason* ("before data-policy review") should be sharpened: research shows the consumer default-train is confirmed but the **API/enterprise data posture is genuinely unknown** — that's the real open item, not a generic "review." **Recommended action:** reword §5.3 FR-2c rationale to "API data posture unverified" and add it to Open Questions §9. (Substantive accuracy, not scope.)
