...
- **Two concrete things have gone stale and need editing.** (1) The **iOS haptics shim** (`<input type="checkbox switch">` + label `.click()`) recommended in §4.4 was **patched in iOS 26.5** and no longer fires programmatically — the PRD's named iOS haptics technique is now dead. (2) The **TanStack Virtual "weak for chat"** claim in §4.5/§9 is now outdated: TanStack shipped first-class chat support (`anchorTo: 'end'`, `followOnAppend`) and explicitly recommends it for chat as of a **May 25, 2026** post.
...
1. **[error] §4.4 Haptics — the iOS shim is dead.** The PRD recommends "`<input type="checkbox switch">` + label-`click()` shim (iOS 18+)" for iOS haptics. **Apple patched the programmatic label-`.click()` haptic trigger in iOS 26.5** (current May 2026); haptics now only fire on a *direct user* toggle, not programmatically. (github.com/tijnjh/ios-haptics; mdn browser-compat-data issue 29166 — accessed 2026-05-27)
...
2. **[error] §4.5 + §9 #1 — TanStack Virtual is no longer "weak for chat."** PRD states TanStack is "explicitly weak for bidirectional/chat per its own maintainers; treat as fallback, not the default." As of a **May 25, 2026** TanStack post, the library ships `anchorTo: 'end'` + `followOnAppend` with streaming size-delta handling and is explicitly recommended for chat. (tanstack.com/blog/tanstack-virtual-chat — accessed 2026-05-27)
