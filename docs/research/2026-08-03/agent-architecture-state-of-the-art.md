...
| Compute not yet matched between arms | Neither — run the A/B | At matched **thinking-token** budgets single-agent was best or statistically indistinguishable from the best at every budget above the lowest, across three model families and two datasets; the matched quantity excludes prompts and answers, and the corruption experiment that favours multi-agent is one model, one dataset, one budget [33] | **[medium]** |
