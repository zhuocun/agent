...
§4.5 search is a UI feature over an index (title + maybe full-text). In 2026, Claude exposes **chat search + memory as RAG tool-calls the model itself invokes mid-conversation** ("let me search our past chats…"). Note the tier split: memory is free on all plans, but chat-search (the RAG retrieval) is **paid-plan only** — so the retrieval-over-history capability is the gated, higher-value piece. For our persona and multi-model story, "the assistant can reference your prior chats on request" is a compelling P1 that the current spec doesn't anticipate.
...
- Strongest-verified, load-bearing claims: ChatGPT branching launch (Sep 2025) and the *separate* edit-restriction (reported Mar 20, 2026 — a report date, not a confirmed effective date; the sibling research doc cites Mar 23); Claude free memory + ledger + import (Mar 2026); interactive-visualization rollouts across all three majors (Mar–Apr 2026); MCP Apps UI standard (Jan 2026); unified voice (Nov 2025); Gemini I/O 2026 redesign. All **[Verified]** against the cited sources above.
