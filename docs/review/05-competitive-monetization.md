...
**Headline.** The existing research doc is unusually strong and was compiled the same day (2026-05-27), so most of its prices and the core EU AI Act dates check out. The real value of this review is in: (1) **material monetization-model shifts the docs under-weight** (industry-wide pivot to usage/credit billing; AI-native subscription retention is catastrophically worse than the docs assume); (2) **a whole regulatory layer the docs miss** (US state AI laws live as of Jan 1 2026 + the Dec 2025 Trump preemption EO); (3) **the free-tier-default-model decision should narrow** (the *DeepSeek-hosted API* faces government/enterprise bans across our target markets + an Italy consumer block, making it a poor privacy-first default — though Western-hosted open weights remain a P2+ option); and (4) **two new direct competitors** (T3 Chat, and increasingly capable aggregators) that sit squarely on our wedge.
...
### 1.3 ARPU reality check + the ads picture (mixed, not one-directional)
New, concrete 2026 ARPU bands for AI-chat apps: **ad-only free ≈ $0.40–$2; hybrid freemium ≈ $3–$15; subscription-led ≈ $30–$100+** (annual ARPU). `[Verified — directional]`

**Correction (vs. an earlier draft of this review):** the market is **not** uniformly moving *toward* ads. **Perplexity publicly abandoned ads in Feb 2026, explicitly citing user trust**, and its revenue growth ($200M→$450M+ ARR) is **subscription/enterprise-led, not ad-led** — so the existing docs/PRD §2.2 "Perplexity retreated from ads" rationale is **correct**, not contradicted. The single ARPU-doubling-via-ads figure I had relied on (thrad.ai) is contradicted by the broader record and should not be used. What *is* true: **ChatGPT is expanding ads** (Free + Go, US→intl). So ads are **directionally mixed** — OpenAI leaning in, Perplexity leaning out for trust reasons. `[Verified]`

**Why it matters.** (1) Our subscription-led plan implies an ARPU target near the **$30-100+ band** — only reachable with strong conversion + low churn, which §1.2 says is hard. (2) Deferring ads on a **trust/brand** basis is well-supported (Perplexity is the cautionary-and-then-corrective example, ChatGPT the counter-pressure) — keep the PRD's rationale, just frame ads as "rising at OpenAI but a trust liability we decline," not "the whole market is abandoning ads."
Sources: https://www.thrad.ai/content/arpu-benchmarks-for-ai-chatbot-apps (ARPU bands; its Perplexity-ads claim is **not** reliable) · Perplexity dropped ads Feb 2026: see §3 sources (AIWire/WinBuzzer/Benzinga) · ChatGPT ads expansion: https://www.macrumors.com/2026/02/09/chatgpt-now-has-ads/
...
### 1.8 DeepSeek is a poor fit as our *hosted* free-tier default — narrowing the PRD's open question
DeepSeek (proposed in the PRD as a candidate cheap free-tier default) faces **government-device / public-sector / enterprise bans** across the US, South Korea, Australia, Taiwan, India, plus an **Italy consumer app-store removal** — these are mostly *not* blanket consumer bans, so calibrate the claim. It stores prompts on **servers in China** and is flagged for **GDPR-compliance and data-exfiltration concerns**. `[Verified — scope: mostly government/enterprise + Italy consumer]`

**Important counter-signal:** Perplexity itself **routes DeepSeek R1 via US/EU-hosted weights** — i.e., the *open weights run on Western infrastructure* are a different (and usable) thing from the *DeepSeek-hosted API*. So the honest conclusion is narrower than "banned everywhere": **don't route to the DeepSeek-hosted API as a privacy-first default; self-hosted/Western-hosted open weights remain a P2+ option.** `[Verified]`

**Why it matters.** Routing privacy-first, EU-facing power users' free traffic through the **DeepSeek-hosted API** would contradict our positioning and create regulatory liability. The PRD lists "DeepSeek vs Gemini Flash" as an open question (§9.5) without flagging that the **DeepSeek-hosted API is a poor fit** for a privacy-first Western product (the open weights on Western infra are a separate, P2+ option). Practical default candidates: **Gemini 2.x/3.x Flash, GPT-5.x mini/nano, Claude Haiku-class, or Mistral (EU-hosted)** — not the DeepSeek-hosted API.
...
- **EU AI Act Article 50 transparency applies from Aug 2, 2026; AI-content marking grace period → Dec 2, 2026.** Confirmed against artificialintelligenceact.eu and EC guidance. **Caveat `[VERIFY]`:** a **May 7, 2026 Council/Parliament provisional agreement** is reshuffling several AI-Act deadlines, so these dates are now *provisional pending Official Journal publication* — re-confirm before locking compliance timelines. `[Verified base dates; VERIFY re May-2026 amendment]`
  - Note: PRD 04 §5.7 and PRD 00 §10 carry a *different* split (content-marking ~Dec 2026 treated as a separate later obligation). The architecture review (`docs/review/03-architecture.md` §3.1) argues Article 50(2) content-marking actually also binds **Aug 2, 2026** — this conflict between the two reviews must be resolved with a lawyer before relying on either date.
...
- **"Perplexity has largely abandoned ads."** **CONFIRMED (existing docs are right).** Perplexity publicly dropped ads in Feb 2026 citing user trust; its growth is subscription/enterprise-led. The PRD's "Perplexity retreated from ads" rationale stands. The only nuance to add: this is **not** a whole-market trend — **ChatGPT is *expanding* ads** (Free + Go, US→intl), so frame ads as "a trust liability we decline even as OpenAI leans in," not "the market is abandoning ads." `[Verified]`
...
- **"DeepSeek vs Gemini Flash" as a balanced open question for the free default.** **Narrowed** — the *DeepSeek-hosted API* is a poor fit for our positioning/markets (government/enterprise bans + Italy consumer block); Western-hosted open weights remain a P2+ option (§1.8). `[Verified]`
...
2. *(Withdrawn — earlier draft wrongly flagged the docs' "Perplexity abandoned ads" line as stale. It is **correct**: Perplexity dropped ads Feb 2026 for trust reasons. The only addition is that ChatGPT is expanding ads, so the market is mixed — see §2.3.)*
...
- **[Rationale mostly stands — refine] §2.2 — "Not shipping ads … Perplexity retreated from it."** This is **correct** — Perplexity dropped ads Feb 2026 for trust reasons (§2.3). Keep the rationale; just add the nuance that **ChatGPT is expanding ads**, so the framing should be "ads are a trust liability we decline even as OpenAI leans in," not "the whole market is abandoning ads."
- **[Open question — narrowed] §9.5 / §5.4.1 — free-tier default "DeepSeek vs Gemini Flash."** The DeepSeek-*hosted API* faces government/enterprise bans across our target markets + an Italy consumer block and contradicts our privacy story (§1.8). The PRD should **remove the DeepSeek-hosted API as a default** and pick from Gemini Flash / GPT-mini / Claude Haiku / Mistral (EU). Self-hosted / Western-hosted DeepSeek open weights = P2+ only.
...
- **[Verify dates — keep the `[VERIFY]` flag] §7.5 / §4.4 — EU AI Act Aug 2/Dec 2 2026.** The base dates check out, **but do NOT downgrade the `[VERIFY]` flag to "confirmed":** a **May 7, 2026 Council/Parliament provisional agreement** is reshuffling AI-Act deadlines (provisional pending Official Journal publication). Separately, the architecture review (`docs/review/03-architecture.md` §3.1) contends Article 50(2) **content-marking also binds Aug 2, 2026** (not Dec 2) — an unresolved conflict between the two reviews. Both the date amendment and the Aug-vs-Dec content-marking question need legal confirmation before lock. The €35M/7% penalty figure is consistent with the Act. `[VERIFY]`
...
- **§9.5 (free default model):** narrowed — drop the **DeepSeek-hosted API** as a default; Western-hosted open weights stay a P2+ option (§1.8).
- **§9.10 (ads at scale):** the question is brand/trust — ads are a **mixed** market signal (ChatGPT expanding; Perplexity withdrew for trust). Our deferral on trust grounds is well-supported; revisit only at scale.
...
4. **Drop the DeepSeek-hosted API as the free-tier default and fix the privacy-claim precision.** The DeepSeek-hosted API faces government/enterprise bans across our target markets (+ Italy consumer block) and contradicts the privacy story — default to Gemini Flash / GPT-mini / Claude Haiku / Mistral instead (Western-hosted open weights stay a P2+ option). Separately, reword the "no legal confidentiality" point to its accurate (district-court, privilege-only) scope and market "minimal retention," not "confidential." (§1.7, §1.8, §3.3)
