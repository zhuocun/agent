  ...
  return (
    ...
    >
      <body className="min-h-full flex flex-col">
        ...
        <div
          aria-hidden
          // `chrome-frost` carries exactly this filter — the glass blur and
          // saturation, deliberately without the `brightness(--glass-brightness)`
          // lift the `glass-*` utilities apply. That lift (1.03) is
          // sub-perceptual over colored content but pushes the near-white app
          // surface (#f9fafc) PAST the top of the gamut, every channel clipping
          // to 255, so over the page this strip rendered a pure-white band that
          // read brighter than the body. Taking the filter from the class, not
          // from an inline style, is what lets the reduced-transparency and
          // `@supports` blocks in `globals.css` drop the blur here.
          className="chrome-frost pointer-events-none fixed inset-x-0 top-0 z-[100]"
          style={{
